# Estrattore Metadati DICOM - Archivio Esami RX
...
- **Traduzione parti anatomiche** - Traduce le descrizioni in inglese dal macchinario in italiano (es. `FOOT` → `PIEDE`, `RIB` → `COSTOLA`, `CERVICAL SPINE` → `COLONNA CERVICALE`). Rimuove i suffissi tecnici del macchinario (`STYLE_*`, `ORTO`, `ERECT`, ecc.) e normalizza le proiezioni (`LAT` → `LL`).
...
- **Filtro per anno** - Prompt interattivo o flag `--year` da riga di comando per selezionare quale/i anno/i esportare.
- **Filtro per modalita** - Elabora solo le modalita radiografiche (DX, CR, RF, XA, MG, DR, PX, IO, OP) e ignora SR/KO/PR/SC.
- **Riepilogo mensile** - Un secondo foglio Excel ("Riepilogo Mensile") con una tabella incrociata del conteggio esami per tipo e per mese, ordinata per totale, con riga e colonna TOTALE.
- **Output Excel formattato** - Intestazioni stilizzate, filtro automatico, blocco riquadri, larghezze colonne appropriate e gestione errore file bloccato (salvataggio automatico su percorso alternativo se il file e aperto).
- **Riepilogo in console** - Stampa il dettaglio esami per mese e le statistiche per fascia d'eta dopo l'esportazione.
...
**Filtro per anno** (salta il prompt interattivo):
```bash
python3 dicom_to_excel.py /percorso/cartella_OsiriX --year 2024
python3 dicom_to_excel.py /percorso/cartella_OsiriX --year 2023,2024
```
...
| Colonna | Descrizione |
|---|---|
| DATA ESAME | Data dell'esame (GG/MM/AAAA) |
